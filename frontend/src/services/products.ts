...
export async function fetchProducts(params: FetchProductsParams): Promise<FetchProductsResponse> {
  ...
  const res = await apiFetch(url.toString());
    ...
    const isWeb = typeof navigator !== 'undefined' && (navigator as any).product === 'Gecko' || typeof window !== 'undefined';
    // Prefer .webp on web when available (generated by optimize-images), fallback to .png on native
    const img = (base: string) => {
      if (isWeb) {
        try {
          return require(`@assets/product-images/${base}.webp`);
        } catch {}
      }
      return require(`@assets/product-images/${base}.png`);
    };
    const categoryImages: Record<string, any> = {
      Electronics: img('Computer'),
      Fashion: img('canvas shoe'),
      Home: img('bag'),
      Beauty: img('camera'),
      Accessories: img('headphone'),
      Automotive: img('car'),
    };
